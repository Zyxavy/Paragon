    ...
    <div class="flex rounded-lg border border-outline-variant/25 overflow-hidden text-xs font-body">
      <button
        ...
        class="px-2.5 py-1 cursor-pointer transition-colors duration-150
               {mode === 'write' ? 'bg-primary text-on-primary' : 'bg-surface text-muted-foreground hover:text-on-surface'}"
      >
        Write
      </button>
      <button
        ...
        class="px-2.5 py-1 cursor-pointer transition-colors duration-150
               {mode === 'preview' ? 'bg-primary text-on-primary' : 'bg-surface text-muted-foreground hover:text-on-surface'}"
      >
        Preview
      </button>
    </div>
